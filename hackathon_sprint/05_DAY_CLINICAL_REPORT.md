# Day 7: Clinical Validation Report

## Deadline
- **Day 7 full day:** Generate comprehensive clinical metrics
- **STOP when:** PDF report with sensitivity/specificity/ROC/CI is ready

## Goal
Create a clinical-grade validation report that goes into the PPT as proof of clinical utility.

---

## Task 1: Run Full External Validation (morning, 3 hours)

Run the validation script from Day 3-4 if not already done:
...
mkdir -p hackathon_sprint/metrics
...
### Verify Output
```bash
cat hackathon_sprint/metrics/external_validation.json | python -m json.tool | head -30
cat hackathon_sprint/metrics/external_validation.txt
```

---

## Task 2: Create Clinical PDF Report Generator (afternoon, 3 hours)

### Create: `scripts/generate_clinical_report.py`

This generates a polished PDF that can be printed or included in the PPT.
...
### Output
- `hackathon_sprint/clinical_validation_report.pdf` — printable clinical report

---

## Task 3: Validate the Report (30 minutes)

- [ ] PDF opens without errors
- [ ] All numbers match `external_validation.json`
- [ ] Confusion matrix is correct
- [ ] CI values are reasonable (not too wide, not too narrow)
- [ ] Disclaimer text is present
- [ ] Report is professional enough for PPT screenshots
...
| Check | Status |
|---|---|
| `external_validation.json` exists | Required |
| `external_validation.txt` exists | Required |
| `clinical_validation_report.pdf` generated | Required |
| Numbers in PDF match JSON | Required |
| PDF is professional quality | Required |
